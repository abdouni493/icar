...
export const Login: React.FC<LoginProps> = ({ lang, onLogin }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-saas-bg via-saas-bg to-[#F3EAD0]">
      ...
      >
        <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-10 space-y-10 border border-saas-border shadow-xl">
          ...
          {/* Créer un compte administrateur — visible UNIQUEMENT tant qu'aucun
              admin n'existe ; disparaît définitivement une fois l'admin créé. */}
          {!adminExists && !isSigningUp && (
            ...
            >
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-sm bg-linear-to-r from-[#E4C878] to-[#B8912E] text-[#14130E] shadow-md hover:shadow-lg transition-all"
              >
                <UserIcon size={17} />
                {lang === 'fr' ? 'Créer un compte administrateur' : 'إنشاء حساب مدير'}
              </button>
              <p className="text-xs text-saas-text-muted">
                {lang === 'fr' ? '(première connexion uniquement)' : '(الاتصال الأول فقط)'}
              </p>
            </motion.div>
          )}

          {/* Revenir à la connexion depuis le mode création de compte */}
          {isSigningUp && (
            <div className="text-center">
              <button
                type="button"
                onClick={() => { setIsSigningUp(false); setErrorMessage(''); }}
                className="text-sm font-semibold text-saas-text-muted hover:text-saas-primary-via transition-colors"
              >
